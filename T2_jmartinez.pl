...
Procesando archivos con Perl
Tarea 2
#Imprime los logs de una manera simplificada
=cut
...
if(defined $opciones{h} or (not defined $opciones{f} and @ARGV != 1)){
	...
	my %lista_patrones = (
		'IP\'s', '\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b',
		'URL\'s', '\bhttps?://[a-z0-9][a-z0-9\-\+\.]{1,61}[a-z0-9]\.[a-z]{2,}[/?\w]*[\.\w]+\??[&?\w=\w]*\b',  #\\.[a-z]{2,}\\??(&[a-z0-9]=\\w)*\\b
		'EMAIL\'s', '\b[a-z0-9][a-z0-9]*[a-z0-9\_\+\.]{1}[a-z0-9]*[a-z0-9]@[a-z0-9][a-z0-9\-\+\.]{1,61}[a-z0-9]\.[a-z]{2,}\b',
		'DOMAIN\'s', '\b[a-z0-9][a-z0-9\-\+\.]{1,61}[a-z0-9]\.[a-z]{2,}\b'
	);	
	...
	if(open my $archivo, '<', $nombre_archivo_entrada){
		...
		while (my $linea = <$archivo>) {
			...
			foreach my $nombre_patron (keys %lista_patrones){
		...
		foreach my $nombre_patron (sort keys %conteo_general){
			...
			# print("$conteo_general{$nombre_patron}");  #HASH(0x7fb39b82d448)
			# print Dumper $conteo_general{$nombre_patron}; #Dumper sí lo muestra correctamente
			# tenemos que de-referenciarlo con %{} ver https://perlmaven.com/multi-dimensional-hashes
